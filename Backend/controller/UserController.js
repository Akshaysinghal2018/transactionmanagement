...
const registerUser= async (req, res , next)=>{
    ...
    if(req.body.typeofuser=="admin"){
        res.json({ "success": false,err:"not register as admin",message:"not register as admin"})
    }
    const saltRounds = 10;
        bcrypt.genSalt(saltRounds, function(err, salt) {
            bcrypt.hash(req.body.password, salt, function(err, hash) {
                if (err) throw err
                data={first_name:firstname,last_name:lastname,email:email,phone_no:mobile,gender:gender,password:hash,type:req.body.typeofuser}
...
const getUser= async (req, res , next)=>{
    ...
    let token=jsonwebtoken.sign({ UID:email },jsonsecret,{ expiresIn: 60*10}) //10 minute expire time for jwt token
...
module.exports= {registerUser,getUser,loginUser,deleteUser,updateUser,forgetPasswordUser,updatePasswordUser,changeprofileimage,addAddress,updateAddress,deleteAddress,getCart,updateCart,checkout,removecart,getorder,getinvoice,autenticateToken,getAllorder}